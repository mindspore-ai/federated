...
FLNAME = "com.mindspore.flclient.demo.albert.AlbertClient"


@pytest.mark.fl_cluster
class TestAlbertTrain(BaseCase):
    """
    ST test class for albert Train
    """
    train_model_path = os.path.join(BaseCase.fl_resource_path, "client/ms/albert_supervise.mindir.ms")
    infer_model_path = os.path.join(BaseCase.fl_resource_path, "client/ms/albert_supervise.mindir.ms")
    ...
    def start_client(self):
        ...
        start_client_cmd = "cd {}/../client_script ;LD_LIBRARY_PATH={} python fl_client_run.py --jarPath={}  " \
            ...
            .format(self.server_path, self.ld_library_path, self.frame_jar_path, self.case_jar_path,
                    self.train_dataset, self.test_dataset, self.vocal_file, self.ids_file, FLNAME,
                    self.train_model_path, self.infer_model_path, self.ssl_protocol, self.deploy_env, self.domain_name,
    ...
    def test_train_albert_nc_ne(self):
        """
        ...
        self.client_num = 1
        self.start_scheduler("yamls/albert/nc_ne_config.yaml")
        self.start_server("yamls/albert/nc_ne_config.yaml", self.cur_ckpt_path)
        self.wait_cluster_ready(out_time=30)
        self.start_client()
        self.check_client_result(out_time=120)

    def test_train_albert_compress_ne(self):
        """
        Feature: FL train process
        ...
        print("Class:{}, function:{}".format(self.__class__.__name__, inspect.stack()[1][3]), flush=True)
        self.client_num = 2
        self.start_scheduler("yamls/albert/compress_ne_config.yaml")
        self.start_server("yamls/albert/compress_ne_config.yaml", self.cur_ckpt_path)
        self.wait_cluster_ready(out_time=30)
        self.start_client()
        self.check_client_result(out_time=120)

    def test_train_albert_nc_dp(self):
        """
        Feature: FL train process
        ...
        print("Class:{}, function:{}".format(self.__class__.__name__, inspect.stack()[1][3]), flush=True)
        self.client_num = 3
        self.start_scheduler("yamls/albert/nc_dp_config.yaml")
        self.start_server("yamls/albert/nc_dp_config.yaml", self.cur_ckpt_path)
        self.wait_cluster_ready(out_time=30)
        self.start_client()
        self.check_client_result(out_time=600)

    def test_train_albert_nc_pw(self):
        """
        Feature: FL train process
        ...
        print("Class:{}, function:{}".format(self.__class__.__name__, inspect.stack()[1][3]), flush=True)
        self.client_num = 4
        self.start_scheduler("yamls/albert/nc_pw_config.yaml")
        self.start_server("yamls/albert/nc_pw_config.yaml", self.cur_ckpt_path)
        self.wait_cluster_ready(out_time=30)
        self.start_client()
        self.check_client_result(out_time=600)

    def test_train_albert_nc_signds(self):
        """
        Feature: FL train process
        ...
        print("Class:{}, function:{}".format(self.__class__.__name__, inspect.stack()[1][3]), flush=True)
        self.client_num = 4
        self.start_scheduler("yamls/albert/nc_signds_config.yaml")
        self.start_server("yamls/albert/nc_signds_config.yaml", self.cur_ckpt_path)
        self.wait_cluster_ready(out_time=30)
        self.start_client()
        self.check_client_result(out_time=600)
...
@pytest.mark.fl_cluster
class TestAlbertInference(BaseCase):
    """
    ST test class for albert infer
    """
    train_model_path = os.path.join(BaseCase.fl_resource_path, "client/ms/albert_supervise.mindir.ms")
    infer_model_path = os.path.join(BaseCase.fl_resource_path, "client/ms/albert_supervise.mindir.ms")
    train_dataset = os.path.join(BaseCase.fl_resource_path, "client/data/albert/train/0.txt")
    ...
    def start_client(self):
        ...
        start_client_cmd = "cd {}/../client_script ;LD_LIBRARY_PATH={} python fl_client_run.py --jarPath={}  " \
                           ...
                           "--batch_size={} --task={}" \
            .format(self.server_path, self.ld_library_path, self.frame_jar_path, self.case_jar_path,
                    self.train_dataset, self.test_dataset, self.vocal_file, self.ids_file,
