...
for i in range(local_server_num):
    ...
    cmd_server += (
        "cd ${execute_path}/logs/server_"
        + str(http_port)
        + "/ || exit && export GLOG_v=1 &&"
    )
    cmd_server += "python ${self_path}/../../run_cloud_deepfm.py"
    ...
    cmd_server += " --http_server_address=" + (
        str(http_server_ip) + ":" + str(http_port)
    )
    cmd_server += " --mindir_model_path=" + mindir_model_path
