...
parser.add_argument(
    "--mindir_model_path", type=str, default="deepfm_ms_train_v2_1.8_32.mindir"
)
...
ssl_config = SSLConfig(
    server_password="server_password_12345", client_password="client_password_12345"
)
...
def get_trainable_params(network):
    ...
    for param in network.trainable_params():
        ...
        if (param_np.dtype != np.float32
                or param.name == "learning_rate"
                or "vhat." in param.name
                or "moment" in param.name
                or "beta" in param.name):
            continue
...
def start_one_server():
    ...
    mindir_model_path = args.mindir_model_path
...
def start_one_scheduler():
    ...
    job = FlSchedulerJob(
        yaml_config=yaml_config,
        manage_address=scheduler_manage_address,
        ssl_config=ssl_config,
    )
    job.run()
