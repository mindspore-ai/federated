...
if __name__ == "__main__":
    ...
    for i in range(local_worker_num):
        os.environ['MS_NODE_ID'] = "fl-worker-" + str(i)
        cmd_server = "execute_path=$(pwd) && self_path=$(dirname \"${script_self}\") && "
        cmd_server += "rm -rf ${execute_path}/worker_" + str(i) + "/ &&"
        cmd_server += "mkdir ${execute_path}/worker_" + str(i) + "/ &&"
        cmd_server += "cd ${execute_path}/worker_" + str(i) + "/ || exit && export GLOG_v=1 &&"
        cmd_server += "python ${self_path}/../run_hybrid_train.py"
        cmd_server += " --device_target=" + device_target
        cmd_server += " --server_mode=" + server_mode
        cmd_server += " --ms_role=MS_WORKER"
