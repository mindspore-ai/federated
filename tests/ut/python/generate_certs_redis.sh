...
rm -rf ${BASEPATH}/fl_redis_ssl_cert && mkdir ${BASEPATH}/fl_redis_ssl_cert
cd ${BASEPATH}/fl_redis_ssl_cert
...
openssl req -new -key serverkey.pem -out serverreq.pem -subj "/C=CN/ST=Some-State/O=FL/CN=redis-server"
...
openssl req -new -key clientkey.pem -out clientreq.pem -subj "/C=CN/ST=Some-State/O=FL/CN=redis-client"
