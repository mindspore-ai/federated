...
from common import get_default_ssl_config
...
from common import start_redis_with_ssl, get_default_redis_ssl_config
...
def test_ssl_config_three_server_two_client_one_iterations_success():
    ...
    data_size = 16
    fl_id = "fl_id_xxxx1"
    start_fl_job_expect_success(http_server_address, fl_name, fl_id, data_size, verify=ca_cert_path)

    # start fl job second for fl_id2, visit server2
    data_size2 = 8
    fl_id2 = "fl_id_xxxx2"
    start_fl_job_expect_success(http_server_address2, fl_name, fl_id2, data_size2, verify=ca_cert_path)
    ...
    client_feature_map, _ = get_model_expect_success(http_server_address, fl_name, iteration, verify=ca_cert_path)
    check_feature_map(expect_feature_map, client_feature_map)
    # get model from sever2
    client_feature_map, _ = get_model_expect_success(http_server_address2, fl_name, iteration, verify=ca_cert_path)
    check_feature_map(expect_feature_map, client_feature_map)
    # get model from sever3
    client_feature_map, _ = get_model_expect_success(http_server_address3, fl_name, iteration, verify=ca_cert_path)
    check_feature_map(expect_feature_map, client_feature_map)
...
@fl_test
def test_ssl_config_redis_server_with_ssl():
    """
    Feature: Server
    Description: Error server password, error client password
    Expectation: Exception will be raised
