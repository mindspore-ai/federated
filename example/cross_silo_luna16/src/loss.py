...
# ============================================================================
"""start calculate cross silo loss"""
...
class CrossEntropyWithLogits(MyLoss):
    ...
    def __init__(self):
        ...
        self.softmax_cross_entropy_loss = nn.SoftmaxCrossEntropyWithLogits(sparse=False, reduction="mean")
        self.cast = F.Cast()
    ...
    def construct(self, logits, label):
        ...
        n, _, _, _ = F.Shape()(label)

        loss = self.softmax_cross_entropy_loss(self.reshape_fn(logits, (n, -1)), self.reshape_fn(label, (n, -1)))
        return loss
