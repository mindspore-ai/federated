...
class BboxAssignSample(nn.Cell):
    ...
    def __init__(self, config, batch_size, num_bboxes, add_gt_as_proposals):
        ...
        self.cast = ops.Cast()
        self.logical_land = ops.LogicalAnd()
        self.less = ops.Less()
    ...
    def construct(self, gt_bboxes_i, gt_labels_i, valid_mask, bboxes, gt_valids):
        ...
        neg_sample_iou_mask = self.logical_land(self.greaterequal(max_overlaps_w_gt, self.zero_thr),
        ...
        for j in range(self.num_gts):
            ...
            pos_mask_j = self.logical_land(self.greaterequal(max_overlaps_w_ac_j, self.min_pos_iou),
        ...
        num_pos = self.cast(self.logical_not(valid_pos_index), self.ms_type)
        num_pos = self.sum_inds(num_pos, -1)
        unvalid_pos_index = self.less(self.range_pos_size, num_pos)
        valid_neg_index = self.logical_land(self.concat((self.check_neg_mask, unvalid_pos_index)), valid_neg_index)
