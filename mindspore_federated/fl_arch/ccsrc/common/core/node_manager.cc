...
void NodeManager::InitNode() {
  initial_total_node_num_ = FLContext::instance()->cluster_config().initial_server_num +
                            FLContext::instance()->cluster_config().initial_worker_num;
  meta_data_ = std::make_unique<ClusterMetadata>(FLContext::instance()->cluster_config().initial_worker_num,
                                                 FLContext::instance()->cluster_config().initial_server_num);
...
void NodeManager::UpdateCluster(bool is_cluster_ready) {
  ...
  for (auto it = heartbeats_.begin(); it != heartbeats_.end(); ++it) {
    if (it->second.tv_sec + FLContext::instance()->cluster_config().heartbeat_timeout < current_time.tv_sec) {
