...
void AbstractNode::Register(const std::shared_ptr<TcpClient> &client) {
  ...
  register_message.set_fl_iteration_num(FLContext::instance()->fl_iteration_num());
...
void AbstractNode::SendFailMessageToScheduler(const std::string &node_role, const std::string &event_info) {
  ...
  std::string now_time = fl::core::CommUtil::GetNowTime().time_str_mill;
...
void AbstractNode::RegisterEventCallback(const fl::core::ClusterEvent &event, const EventCallback &event_cb) {
...
std::shared_ptr<CommunicatorBase> AbstractNode::GetOrCreateTcpComm(const std::string &scheduler_ip,
  ...
  if (!communicators_.count(kTcpCommunicator)) {
    ...
    FLContext::instance()->cluster_config().scheduler_ip = scheduler_ip;
    FLContext::instance()->cluster_config().scheduler_port = static_cast<uint16_t>(scheduler_port);
    FLContext::instance()->cluster_config().initial_worker_num = worker_num;
    FLContext::instance()->cluster_config().initial_server_num = server_num;
...
void AbstractNode::StartHeartbeatTimer(const std::shared_ptr<TcpClient> &client) {
  ...
  heart_beat_thread_ = std::make_unique<std::thread>([&]() {
    uint32_t connect_interval = FLContext::instance()->cluster_config().connect_interval;
    uint32_t heartbeat_interval = FLContext::instance()->cluster_config().heartbeat_interval * 1000;
...
bool AbstractNode::CheckSchedulerTimeout() const {
  struct timeval current_time {};
  (void)gettimeofday(&current_time, nullptr);
  int64_t old_time = scheduler_time_.tv_sec + FLContext::instance()->cluster_config().scheduler_timeout;
...
void AbstractNode::ProcessHeartbeatResp(const std::shared_ptr<MessageMeta> &meta, const void *data, size_t size) {
  ...
  bool not_enable_recover_node_timeout = is_worker;
...
void AbstractNode::ProcessEvent(const std::shared_ptr<TcpConnection> &conn, const std::shared_ptr<MessageMeta> &meta,
  ...
  if (event == static_cast<uint32_t>(UserDefineEvent::kNodeTimeout)) {
    OnEventCallback(ClusterEvent::NODE_TIMEOUT);
  } else {
...
void AbstractNode::InitNodeNum() {
  worker_num_ = FLContext::instance()->cluster_config().initial_worker_num;
  server_num_ = FLContext::instance()->cluster_config().initial_server_num;
  scheduler_ip_ = FLContext::instance()->cluster_config().scheduler_ip;
  scheduler_port_ = FLContext::instance()->cluster_config().scheduler_port;
...
void AbstractNode::PersistMetaData() {
  ...
  if (config_->Exists(kKeyRecovery)) {
    ...
    clusterConfig.scheduler_ip = this->scheduler_ip();
