...
void FileConfiguration::PersistNodes(const fl::core::ClusterConfig &clusterConfig) const {
  if (!CommUtil::IsFileExists(file_path_)) {
...
void FileConfiguration::PersistFile(const fl::core::ClusterConfig &clusterConfig) const {
  if (!CommUtil::IsFileExists(file_path_)) {
    MS_LOG(WARNING) << "The file path:" << file_path_ << " is not exist. create one";
  }
  ...
  persist_js[kRecoverySchedulerIp] = clusterConfig.scheduler_ip;
