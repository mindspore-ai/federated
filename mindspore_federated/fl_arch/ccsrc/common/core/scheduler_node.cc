...
bool SchedulerNode::Start(const uint32_t &timeout) {
  ...
  config_ = std::make_unique<FileConfiguration>(FLContext::instance()->config_file_path());
  ...
  if (FLContext::instance()->scheduler_manage_port() != 0) {
    MS_LOG(INFO) << "Start the restful scheduler http service, the ip is 127.0.0.1 "
                 << ", the port:" << FLContext::instance()->scheduler_manage_port();
    StartRestfulServer(kLocalIp, FLContext::instance()->scheduler_manage_port(), 1);
  }
  Initialize();
...
void SchedulerNode::RunRecovery() {
  ...
  MS_LOG(INFO) << "The scheduler start run recovery!"
               ...
               << ", the scheduler ip:" << clusterConfig.scheduler_ip
...
void SchedulerNode::Initialize() {
  ...
  if (FLContext::instance()->node_id().empty() && config_->Exists(kNodeId)) {
    node_info_.node_id_ = config_->Get(kNodeId, "");
  } else {
    node_info_.node_id_ = FLContext::instance()->node_id();
  }
...
void SchedulerNode::InitEventTxtFile() {
  ...
  if (CommUtil::CreateDirectory(event_file_path_.c_str())) {
    MS_LOG(INFO) << "Create Directory :" << event_file_path_ << " success.";
  }
  event_txt_file_.open(event_file_path_.c_str(), std::ios::out | std::ios::app);
...
void SchedulerNode::CreateTcpServer() {
  ...
  std::string scheduler_ip = FLContext::instance()->cluster_config().scheduler_ip;
  uint32_t scheduler_port = FLContext::instance()->cluster_config().scheduler_port;
  MS_LOG(INFO) << "scheduler ip: " << scheduler_ip << ", scheduler port: " << scheduler_port;
  server_ = std::make_shared<TcpServer>(scheduler_ip, scheduler_port, config_.get());
  ...
  update_state_thread_ = std::make_unique<std::thread>([&]() {
    ...
    while (!is_finish_.load()) {
      ...
      if (!is_ready_ && (std::chrono::steady_clock::now() - start_time >
                         std::chrono::seconds(FLContext::instance()->cluster_config().cluster_available_timeout))) {
        node_manager_.UpdateClusterState(ClusterState::CLUSTER_EXIT);
      }
      std::this_thread::sleep_for(std::chrono::seconds(FLContext::instance()->cluster_config().heartbeat_interval));
      node_manager_.UpdateCluster(is_ready_);

      if (node_manager_.GetClusterState() == ClusterState::CLUSTER_EXIT) {
        std::this_thread::sleep_for(
          std::chrono::seconds(FLContext::instance()->cluster_config().heartbeat_interval * kHeartbeatTimes));
        is_finish_ = true;
        wait_finish_cond_.notify_all();
      }
    }
...
void SchedulerNode::InitNodeMetaData() {
  ClusterConfig &clusterConfig = FLContext::instance()->cluster_config();
  clusterConfig.scheduler_ip = FLContext::instance()->scheduler_ip();
  clusterConfig.scheduler_port = FLContext::instance()->scheduler_port();
  clusterConfig.initial_worker_num = FLContext::instance()->initial_worker_num();
  clusterConfig.initial_server_num = FLContext::instance()->initial_server_num();
