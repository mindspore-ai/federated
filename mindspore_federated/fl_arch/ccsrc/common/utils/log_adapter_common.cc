...
#define BUFLEN 80
  char buf[BUFLEN];
  (void)memset_s(buf, sizeof(buf), '\0', BUFLEN);
#if defined(_WIN32) || defined(_WIN64)
  time_t time_seconds = time(0);
  struct tm now_time;
  localtime_s(&now_time, &time_seconds);
  (void)snprintf_s(buf, sizeof(buf), BUFLEN, "%d-%d-%d %d:%d:%d", now_time.tm_year + 1900, now_time.tm_mon + 1,
  ...
  (void)snprintf_s(buf + time_str_len, sizeof(buf) - time_str_len, BUFLEN - time_str_len, ".%03ld.%03ld",
                   cur_time.tv_usec / time_convert_unit, cur_time.tv_usec % time_convert_unit);
#endif
