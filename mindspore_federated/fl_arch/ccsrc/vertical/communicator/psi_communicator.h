...
#ifndef MINDSPORE_FL_ARCH_CCSRC_VERTICAL_PSI_COMMUNICATOR_H_
#define MINDSPORE_FL_ARCH_CCSRC_VERTICAL_PSI_COMMUNICATOR_H_
...
namespace mindspore {
namespace fl {
class PsiCommunicator : public AbstractCommunicator {
 public:
  PsiCommunicator() = default;
  ~PsiCommunicator() = default;
 ...
 private:
  ...
  bool ResponseHandler(const std::shared_ptr<std::vector<uint8_t>> &response_msg);

  std::mutex message_received_mutex_;

  std::map<std::string, std::map<std::string, MessageQueuePtr>> message_queues_ = {};
};
}  // namespace fl
}  // namespace mindspore
#endif  // MINDSPORE_FL_ARCH_CCSRC_VERTICAL_PSI_COMMUNICATOR_H_
