...
class MS_EXPORT FLContext {
 public:
  ...
  fl::core::ClusterConfig &cluster_config();
  ...
  void set_feature_maps(std::vector<std::string> weight_fullnames, std::vector<std::vector<float>> weight_datas,
                        std::vector<std::vector<size_t>> weight_shapes, std::vector<std::string> weight_types);
  std::map<std::string, Feature> &feature_maps();
 ...
 private:
  FLContext()
      : is_worker_(false),
        ...
        worker_num_(0),
        server_num_(0),
        scheduler_ip_("0.0.0.0"),
        scheduler_port_(6667),
        role_(kEnvRoleOfServer),
        server_mode_(""),
  ...
  std::unique_ptr<fl::core::ClusterConfig> cluster_config_;
  ...
  std::map<std::string, Feature> feature_maps_;
};
}  // namespace fl
}  // namespace mindspore
