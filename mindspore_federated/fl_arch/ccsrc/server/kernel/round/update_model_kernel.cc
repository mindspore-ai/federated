...
void UpdateModelKernel::InitKernel(size_t threshold_count) {
  ...
  auto first_cnt_handler = [](std::shared_ptr<fl::core::MessageHandler>) {};
  auto last_cnt_handler = [this](std::shared_ptr<fl::core::MessageHandler>) { RunAggregation(); };
...
void UpdateModelKernel::OnLastCountEvent(const std::shared_ptr<fl::core::MessageHandler> &) {}
...
void UpdateModelKernel::RunAggregation() {
  ...
  if (is_last_iter_valid) {
    ...
    if (FLContext::instance()->resetter_round() == ResetterRound::kUpdateModel &&
        FLContext::instance()->encrypt_type() != kPWEncryptType) {
...
ResultCode UpdateModelKernel::VerifyUpdateModel(const schema::RequestUpdateModel *update_model_req,
  ...
  if (FLContext::instance()->encrypt_type() == kPWEncryptType) {
    ...
    armour::CipherMetaStorage cipher_meta_storage;
    cipher_meta_storage.GetClientListFromServer(kCtxGetSecretsClientList, &get_secrets_clients);
...
bool UpdateModelKernel::IsCompress(const schema::RequestUpdateModel *update_model_req) {
  if (FLContext::instance()->upload_compress_type() != kNoCompress &&
...
bool UpdateModelKernel::VerifyUploadCompressFeatureMap(const schema::RequestUpdateModel *update_model_req) {
  ...
  if (upload_sparse_rate != FLContext::instance()->upload_sparse_rate()) {
...
ResultCode UpdateModelKernel::UpdateModel(const schema::RequestUpdateModel *update_model_req,
  ...
  } else if (FLContext::instance()->upload_compress_type() == kDiffSparseQuant) {
...
std::map<std::string, UploadData> UpdateModelKernel::ParseSignDSFeatureMap(
  ...
  float signds_grad = sign * FLContext::instance()->sign_global_lr();
...
void UpdateModelKernel::RecordCompletePeriod(const DeviceMeta &device_meta) {
  ...
  uint64_t update_model_complete_time = fl::core::CommUtil::GetNowTime().time_stamp;
