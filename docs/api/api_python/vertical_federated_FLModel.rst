...
.. py:class:: mindspore_federated.FLModel(role, yaml_data, network, train_network=None, loss_fn=None, optimizers=None, metrics=None, eval_network=None, eval_indexes=None, grad_network=None)
    ...
    参数：
        - **role** (str) - 纵向联邦学习参与方的角色，可取值为'leader'或'follower'。
    ...
    .. py:method:: eval_one_step(local_data_batch=None, remote_data_batch=None)
        ...
        参数：
            - **local_data_batch** (dict) - 从本地服务器读取的数据batch。
            - **remote_data_batch** (dict) - 从其它参与方的远程服务器读取的数据batch。
...
.. py:class:: mindspore_federated.vfl_utils.FLYamlData(path)
